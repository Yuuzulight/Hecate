...
That 36 seconds is the cluster's own startup, not any one pod's. The API pod has a narrower race inside it: it connects to Postgres once, at import time, before uvicorn ever starts serving — and cluster DNS is not always ready that early. Measured live, `postgres.hecate.svc.cluster.local` sometimes did not resolve until 25 to 40 seconds after the container itself started, well past what felt like a generous guess. A connection attempt with no retry treated that as a real outage and crashed the whole pod rather than the one attempt that hit it, and Kubernetes' own restart backoff — which grows, 10 seconds then 20 then 40 — recovered far slower than the DNS record actually needed. The fix is a bounded retry on that one connection: several attempts, several seconds apart, wide enough to cover what was actually measured rather than what seemed like enough margin in advance. `server.py`'s metrics-refresh loop already treated a dropped connection as routine and reconnected; this carries the same tolerance to the one call that only runs once, at the moment the whole pod's fate depends on it.
...
Both used to be hardcoded to Claude. Both were blocked on the same thing at once: Anthropic account credits, separate from and unfunded by the Claude subscription. Gemini's free tier removes that specific blocker, so `RAG_PROVIDER` (`gemini` default / `anthropic` / `openai`) picks the provider for both the answering chain and the judge, built through one shared module, `pipeline/rag/providers.py`. That removes the Anthropic-credits blocker, not every possible one: a Google Cloud project still has its own billing state, and live verification hit exactly that — deployment and provider-selection wiring confirmed correct on `RAG_PROVIDER=gemini`, but the answer itself blocked on that Google Cloud project's prepay credits being exhausted (`429 RESOURCE_EXHAUSTED`), a billing fact about that account rather than a defect here. `RAG_PROVIDER=anthropic` is the one proven end-to-end: a full run of the fixed question set through the real warehouse scored 0.92 mean faithfulness and zero hallucinations, judged by a real second model call against the same context the answer saw.

That run also found two things only real traffic surfaces. The judge's structured verdict truncated on some answers at the token ceiling that had been sized for the answering side, not for a metric that decomposes an answer into individual claims first — a longer answer needing more room to get through the same response shape. And the judge's default sampling parameters, inherited from the library wrapping it rather than chosen here, are exactly the ones Claude Opus 5 has dropped support for — the same constraint the answering chain already worked around, just not yet carried over to the judge because nothing had exercised that path for real. Both are why this evaluation harness is trusted now in a way "it's built" didn't yet earn.
...
Judge calls are billed the same as answer calls - two more real requests per question, on top of the one that answers it - and for a while only the answer's spend was visible anywhere. The judge's client is a raw provider SDK object patched by `instructor`, not the LangChain model the chain uses, so the same token-reading code didn't apply. `instructor` exposes a hook that fires with the raw, unparsed response on every real call it makes; reading usage off that and feeding it into the same `hecate_rag_tokens_total`/`hecate_rag_cost_usd_total` counters the chain already used was the fix, done per provider because each SDK names and nests those fields differently.
