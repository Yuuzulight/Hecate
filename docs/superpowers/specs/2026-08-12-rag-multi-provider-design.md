...
**Known risks, not yet resolved - both need an early smoke test before the rest of the chain is built around them, run manually against real credentials.** `.github/workflows/test.yml` sets no real provider keys at all - existing tests reach `ANTHROPIC_API_KEY` via `monkeypatch.setenv(..., "sk-ant-test")`, a fake value, and CI has no real key for any of the three providers. A CI-green result cannot mean either of these was actually checked, no matter how the automated tests around them are written - this is exactly the failure shape this project has a running list of elsewhere. Run both by hand, once, with real credentials, before considering the implementation done:
